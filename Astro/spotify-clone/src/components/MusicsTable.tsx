...
export const MusicsTable: FC<Props> = ({ songs, playlist }) => {
  ...
  const { song: currentMusic } = currentSong;
  ...
  const handleRowDoubleClick = (index: number, song: Song) => {
    if (currentMusic?.id === song.id) {
      setIsPlaying(!isPlaying);
    } else {
      setCurrentSong({ song: songs[index], songs, playlist: playlist });
      setIsPlaying(!isPlaying);
    }
  };
  ...
  const playSelectedMusic = (index: number, song: Song) => {
    if (currentMusic?.id === song.id) {
      setIsPlaying(!isPlaying);
    } else {
      setCurrentSong({ song: songs[index], songs, playlist: playlist });
      ...
      <tbody>
        {songs.map((song, index) => (
          <tr
            ...
            onDoubleClick={() => handleRowDoubleClick(index, song)}
            onMouseEnter={() => setHoveredRow(index)}
            onMouseLeave={() => setHoveredRow(null)}
          >
            <td className="px-4 py-2">
              {selectedRow === index || hoveredRow === index ? (
                <button onClick={() => playSelectedMusic(index, song)}>
                  <span className="text-white text-right w-2 h-2 text-xs">
                    {song.id === currentMusic?.id && isPlaying ? (
                      <Pause className="size-5 h-5 text-sm" />
                    ) : (
